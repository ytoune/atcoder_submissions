fn main() {
  ...
  dpav[1] = {
    ...
    if av1 < av0 && av2 < av0 {
      Tmp { need: true, ..dpav[0] }
    } else if av0 < av1 && av2 < av1 {
      Tmp { count: 1, total: a[1], av: av1, need: false }
    } else {
      Tmp { count: 2, total: a[1] + a[0], av: av2, need: false }
    }
  };
  for i in 2..n {
    dpav[i] = {
      ...
      if av1 < av0 && av2 < av0 && !dpav[i - 1].need {
        Tmp { need: true, ..dpav[i - 1] }
      } else if av0 < av1 && av2 < av1 && !dpav[i - 2].need {
        Tmp { count: count1, total: total1, av: av1, need: false }
      } else {
        Tmp { count: count2, total: total2, av: av2, need: false }
      }
    };
  }
  println!("{:?}", &dpav);
